    ...
    module ClassMethods
      def historical_fields_for(*attributes)
        ...
        attributes.collect(&:to_s).each do |attribute|
          self.class_eval do
            has_many attribute.pluralize.to_sym, :class_name => 'HistoricalField', :as => :parent, :conditions => [ 'historical_fields.field_name = ?', attribute ], :order => 'historical_fields.created_at ASC'
